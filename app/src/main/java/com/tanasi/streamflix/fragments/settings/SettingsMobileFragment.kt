...
class SettingsMobileFragment : PreferenceFragmentCompat() {
    ...
    companion object {
        private const val TAG = "SettingsMobileFragment"
        private const val DEBUG_PROVIDER_TAG = "SettingsMobileDebug"
    }
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        setupDohPreference()
        updateDohPreferenceVisibility() // Chiamata dopo setup
    }
    ...
    override fun onResume() {
        ...
        updateNetworkSettingsTitle()
        updateStreamingCommunityCategoryVisibility()
        updateDohPreferenceVisibility() // Chiamata per aggiornare la visibilità del DoH
        updateSummaries()
    }
    ...
    private fun updateStreamingCommunityCategoryVisibility() {
        ...
        Log.d(TAG, "StreamingCommunity settings category visibility set to: $isVisible based on provider: ${UserPreferences.currentProvider?.name}")
    }
    ...
    private fun setupDohPreference() {
        ...
        dohPreference.value = UserPreferences.dohProviderUrl ?: UserPreferences.DOH_DISABLED_VALUE
    ...
    private fun updateDohPreferenceVisibility() {
        val dohPreference = findPreference<ListPreference>("p_doh_provider_url")
        if (dohPreference == null) {
            Log.d(TAG, "ListPreference 'p_doh_provider_url' NOT FOUND. Skipping DOH visibility update.")
            return
        }
        val isStreamingCommunity = UserPreferences.currentProvider is StreamingCommunityProvider
        dohPreference.isVisible = isStreamingCommunity
        Log.d(TAG, "DOH preference visibility updated to: $isStreamingCommunity based on provider: ${UserPreferences.currentProvider?.name}")
    }
    ...
    private fun updateSummaries() {
        ...
        val dohPreference = findPreference<ListPreference>("p_doh_provider_url")
        if (dohPreference != null && dohPreference.isVisible) { // Controlla anche la visibilità
        ...
        } else if (dohPreference != null) {
            dohPreference.summary = null // Nascondi sommario se non visibile
        }
